...
bool DataManage::open(const QString &dbPath) {
    if (m_db.isOpen()) {
       return true;
    }
...
bool DataManage::init(const QString &schemaPath) {
...
bool DataManage::executeSqlScript(const QString &filePath)
{
...
QSqlDatabase DataManage::db() const
{
    return m_db;
}
